...
Example
-------
Here is how to parse an input stream::

    parser = BisonParser(regexp, handler, tables,
                         rules.bind_to(builder), special_rule)
    nerrors = parser.parse(open(filename))

This will call the builder methods specified in the grammar rules in
bparser.py in order to parse the file.  Syntax errors will result in
error callbacks to the builder.  The builder must report semantic
errors to the parser by raising SemanticError or AbortParse exceptions
instead of returning.  The parser catches these and initiates the
error recovery specified in the grammar.
